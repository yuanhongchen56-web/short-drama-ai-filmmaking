...
这是一个专为**短剧（竖屏/横屏）、微电影、短视频**等影视创作设计的高级 AI 智能体指令（System Prompts / Gems）套件。该套件包含四个处于核心生态位的专业智能体，构成了从“小说/故事原文”到“AI 绘图/视频生成提示词”的工业级自动化控制管线。

特别适配于 **Seedance 2.0、Nano Banana、Midjourney、Images 2.0** 等主流 AI 视频/图像生成工具。

---

## 🎬 核心智能体介绍

本套件由以下三个 **v3.0 旗舰版** 智能体指令组成，每个文件都包含严格的执行铁律、工作流和状态跟踪锁（State Tracker）：
...
*   **核心模块**：
    *   `A1 文生视频`（严格遵循摄影视角、主体细节、动作演变、美学风格等“三句话”固定结构）。
    *   `A2 图生视频`（应用“减法铁律”，主攻摄影运镜或动作演变，绝对不重复参考图已有内容）。
    *   `A3 批量分镜生成`（支持“批次合并”和“逐镜生成”，支持时间锚点防熔化设计）。
    *   `A4 提示词优化`（诊断维度混乱、堆砌词汇等问题，重构输出高表现力提示词）。
...
```mermaid
graph TD
    A[小说 / 故事原文] -->|1. 剧本改编/洗稿| B(影视剧本改编大师 v3.0)
    B -->|2. 分镜设计与对白| C(分镜总导演 v3.0)
    B -->|3. 角色与场景视觉| D(角色/场景艺术指导 3.0)
    D -->|4. 立绘与概念图提示词| E[AI 绘图工具 Midjourney/Images 2.0]
    C -->|5. 空间基准与参考图绑定| F(AI 视频生成工作流)
    E -->|6. 分镜控制提示词| G[AI 视频工具 Seedance/Images 2.0]
    F -->|6. 五维度去噪控制提示词| G[AI 视频工具: Seedance2.0]
```
...
2.  **执行管线流程**：
    ...
    *   随后使用 **艺术指导** 生成一致的角色和场景垫图。
    *   最后结合 **导演** 与 **AI 视频提示词生成专家**，导入剧本和垫图，利用五维坐标系和去噪控制输出精准的批量视频提示词，一键复制进 AI 视频生成器。

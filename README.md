...
本套件由以下三个 **v6.5 旗舰版** 智能体指令组成，每个文件都包含严格的执行铁律、工作流和状态跟踪锁（State Tracker）：

### 1. 影视剧本改编大师 (`01_screenplay_adaptation_master.md`)
...
### 2. 角色/场景艺术指导 (`02_character_scene_art_director.md`)
...
### 3. 分镜总导演 (`03_storyboard_director.md`)
...
```mermaid
graph TD
    A[小说 / 故事原文] -->|1. 剧本改编/洗稿| B(影视剧本改编大师 v6.5)
...
1.  **创建智能体 (Gems / GPTs / Coze)**：
    ...
    *   推荐使用 **Gemini 1.5 Pro** 或 **GPT-4o** 等具备大上下文和强逻辑遵循能力的模型。
2.  **执行管线流程**：
